...
Sync.prototype.init = function() {
    ...
    // blank metadata we'll use as a writer
    this.meta = new id3.Meta({
        'artist': ''
    });

    // rewrite metadata file
    // with clean tags
    this.writer = new id3.Writer();
...
Sync.prototype.coverToOgg = function(tmp, dest, done) {
    try {
        child.exec('ffmpeg -y -i '+tmp+' -map 0:0 -map_metadata -1 '+dest, function(err, stdout, stderr) {
            done.call(this, dest);
        }.bind(this));
    } catch (e) {
        console.log(e);
